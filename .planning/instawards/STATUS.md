...
Last updated: 2026-06-17 (Jobs 0–7 ALL DONE. Evidence package `EVIDENCE-SUBMISSION.md` ready. Only human steps left: tell Kenny re VPS deviation + hand him the doc for Airtable)
...
| 6 | Demo videos (CLI flow + site walkthrough) | `06-demo-video.md` | ✅ **DONE** (both on YouTube) | #2, #4, #5 |
| 7 | Evidence submission package for Ambassador | `07-evidence.md` | ✅ **DONE** (`EVIDENCE-SUBMISSION.md`; awaiting hand-off to Kenny) | #1–#6 |
...
|------|-----------|---------|
| 2026-06-17 | Job #7: evidence package | **All evidence compiled → `EVIDENCE-SUBMISSION.md`** (self-contained, maps SOW §6 D1/D2/D3 to live proof). Verified live this session: repo 200, **npm v0.1.1** (SOW evidence updated from stale 0.1.0), both in-scope contracts confirmed on-chain via stellar.expert API (deployed 2026-04-01, deployer `GDQZEOCX…`), site routes + API 200, 2 YouTube videos. Included **disclosure note** (Vercel→VPS, `.xyz`→`.dev`) + **bonus** (Reputation/Passport/MultiWallet deployed ahead of scope). Job 7 ✅. **Remaining = human-only:** (1) Pebri tells Kenny re VPS deviation; (2) hand `EVIDENCE-SUBMISSION.md` to Kenny → Airtable; (3) ensure both YouTube videos are Unlisted/Public (not Private). |
| 2026-06-17 | Job #6: demo videos | **Both demo videos recorded by builder** (Focusee → Loom/YouTube). Reviewed SOW §6 evidence — confirmed the 2 videos match D2/D3 requirements verbatim. Dry-ran the CLI flow live (warmed npx cache): agent **#60** OK. Builder then recorded **Video 1 (CLI)** live → agent **#61** `GDL4SHVX…7M34`, register `8d01c08c…`, verify `4047953f…`, lookup ✅ basic; **verified live** (API `verified:true`, profile 200, in directory). **Video 2 (site)** = landing → directory search/filter → agent #61 profile w/ VERIFIED badge → docs. Clarified for builder: **security audit = out-of-scope** (Phase 2, not required — D1 needs only the >80% coverage report, already 96.33%); **`Passport ❌` is expected** (Soulbound Passport = Phase 2). **DONE:** both videos published on YouTube — Video 1 (CLI/D2): https://www.youtube.com/watch?v=JQqOvmD348w · Video 2 (site/D3): https://www.youtube.com/watch?v=ioPVA0R-9VE. Job 6 ✅. Next: Job 7 (evidence package + Vercel→VPS deviation note for Kenny). |
